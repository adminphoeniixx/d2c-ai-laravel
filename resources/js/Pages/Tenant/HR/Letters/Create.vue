...
<TenantLayout>
    ...
    <form @submit.prevent="submit" class="max-w-3xl space-y-5">
        <div class="card">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    ...
                    <select v-model="form.employee_id" class="pulsara-input">
                ...
                <div>
                    <label class="pulsara-label">Type</label>
                    <select v-model="form.type" class="pulsara-input">
                        <option v-for="(label, type) in typeLabels" :key="type" :value="type">{{ label }}</option>
                    </select>
                </div>
                <div>
                    <label class="pulsara-label">Use Template</label>
                    <select v-model="form.letter_template_id" class="pulsara-input">
                ...
                <div>
                    <label class="pulsara-label">Title</label>
                    <input v-model="form.title" class="pulsara-input" />
        ...
        <div>
            <label class="pulsara-label">Letter Body (HTML — placeholders will be replaced with employee data)</label>
            <textarea v-model="form.body" class="pulsara-input font-mono text-[12px]" rows="25" placeholder="Write or paste your letter content here, or select a template above..."></textarea>
