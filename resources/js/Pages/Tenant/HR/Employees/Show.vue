...
import { ref } from 'vue';
import { ArrowLeft, Edit, FileText, Plus, Mail, Phone, MapPin, Building2, Calendar, CreditCard, IndianRupee, Upload, Trash2, File, X } from 'lucide-vue-next';
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="card">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-[13px]">
                    <div><span class="pulsara-label">Designation</span><div class="text-ink">{{ employee.designation || '—' }}</div></div>
                    <div><span class="pulsara-label">Department</span><div class="text-ink">{{ employee.department || '—' }}</div></div>
                    <div><span class="pulsara-label">Joined</span><div class="text-ink">{{ dateFmt(employee.date_of_joining) }}</div></div>
                    <div><span class="pulsara-label">Type</span><div class="text-ink capitalize">{{ employee.employment_type?.replace('_', ' ') }}</div></div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-[13px]">
                    <div><span class="pulsara-label">Annual CTC</span><div class="text-white font-mono font-semibold text-[15px]">{{ fmt(employee.ctc_annual) }}</div></div>
                    <div><span class="pulsara-label">Basic Salary</span><div class="text-ink font-mono">{{ fmt(employee.basic_salary) }}</div></div>
                    <div><span class="pulsara-label">HRA</span><div class="text-ink font-mono">{{ fmt(employee.hra) }}</div></div>
                    <div><span class="pulsara-label">Special Allowance</span><div class="text-ink font-mono">{{ fmt(employee.special_allowance) }}</div></div>
                    <div><span class="pulsara-label">Other Allowance</span><div class="text-ink font-mono">{{ fmt(employee.other_allowance) }}</div></div>
                    <div><span class="pulsara-label">Monthly Total</span><div class="text-emerald font-mono font-semibold">{{ fmt(parseFloat(employee.basic_salary || 0) + parseFloat(employee.hra || 0) + parseFloat(employee.special_allowance || 0) + parseFloat(employee.other_allowance || 0)) }}</div></div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-[13px]">
                    <div><span class="pulsara-label">Bank</span><div class="text-ink">{{ employee.bank_name || '—' }}</div></div>
                    <div><span class="pulsara-label">Account No.</span><div class="text-ink font-mono">{{ employee.bank_account_number || '—' }}</div></div>
                    <div><span class="pulsara-label">IFSC</span><div class="text-ink font-mono">{{ employee.bank_ifsc || '—' }}</div></div>
                    <div><span class="pulsara-label">PAN</span><div class="text-ink font-mono">{{ employee.pan_number || '—' }}</div></div>
                    <div><span class="pulsara-label">Aadhaar</span><div class="text-ink font-mono">{{ employee.aadhaar_number || '—' }}</div></div>
                    <div><span class="pulsara-label">UAN (PF)</span><div class="text-ink font-mono">{{ employee.uan_number || '—' }}</div></div>
                </div>
            </div>
                ...
                <div v-if="employee.documents?.length">
                    <div v-for="doc in employee.documents" :key="doc.id"
                        ...
                        <div class="flex items-center gap-3 min-w-0">
                            ...
                            <div class="min-w-0">
                                <a :href="doc.file_url" target="_blank" class="text-[13px] font-medium text-white hover:text-brand-300 transition truncate block">{{ doc.name }}</a>
                                <div class="text-[10px] text-ink-3">{{ doc.type?.replace('_', ' ') }} · {{ doc.file_size || '—' }} · {{ doc.file_name }}</div>
                            </div>
                        </div>
                        <button class="text-ink-3 hover:text-rose flex-shrink-0 cursor-pointer" @click="deleteDoc(doc)"><Trash2 :size="14" /></button>
                    </div>
                </div>
            ...
            <div v-if="showDocUpload" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="showDocUpload = false">
                <div class="card w-full max-w-md mx-4">
                    ...
                    <form @submit.prevent="uploadDoc" class="space-y-3">
                        <div><label class="pulsara-label">Document Name *</label><input v-model="docForm.name" class="pulsara-input" placeholder="Aadhaar Card" /><div v-if="docForm.errors.name" class="mt-1 text-[11px] text-rose">{{ docForm.errors.name }}</div></div>
                        <div><label class="pulsara-label">Type</label>
                            <select v-model="docForm.type" class="pulsara-input">
                                <option v-for="t in docTypes" :key="t" :value="t">{{ t.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="pulsara-label">File *</label>
                            <input type="file" @change="onFileSelect" class="pulsara-input text-[12px]" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" />
                        ...
                        <div><label class="pulsara-label">Notes</label><input v-model="docForm.notes" class="pulsara-input" placeholder="Optional notes…" /></div>
