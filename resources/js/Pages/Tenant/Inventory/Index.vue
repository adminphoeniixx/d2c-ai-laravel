...
<TenantLayout>
    ...
    <div class="flex gap-3 mb-4">
        <div class="relative flex-1">
            ...
            <input v-model="searchQ" @keyup.enter="doSearch" class="pulsara-input pl-9" placeholder="Search by name or SKU…" />
        </div>
    </div>
        ...
        <div class="card w-full max-w-lg mx-4 max-h-[85vh] overflow-y-auto">
            ...
            <form @submit.prevent="submit" class="space-y-3">
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="pulsara-label">Name *</label><input v-model="form.name" class="pulsara-input" /><div v-if="form.errors.name" class="mt-1 text-[11px] text-rose">{{ form.errors.name }}</div></div>
                    <div><label class="pulsara-label">SKU *</label><input v-model="form.sku" class="pulsara-input font-mono" /><div v-if="form.errors.sku" class="mt-1 text-[11px] text-rose">{{ form.errors.sku }}</div></div>
                </div>
                <div class="grid grid-cols-3 gap-3">
                    <div><label class="pulsara-label">Category</label><input v-model="form.category" class="pulsara-input" placeholder="Raw Material" /></div>
                    <div><label class="pulsara-label">Unit</label><select v-model="form.unit" class="pulsara-input"><option v-for="u in ['pcs','kg','litre','box','m','pair']" :key="u" :value="u">{{ u }}</option></select></div>
                    <div><label class="pulsara-label">Location</label><input v-model="form.location" class="pulsara-input" placeholder="Rack A1" /></div>
                </div>
                <div class="grid grid-cols-3 gap-3">
                    <div><label class="pulsara-label">Quantity</label><input v-model.number="form.quantity" type="number" class="pulsara-input" /></div>
                    <div><label class="pulsara-label">Cost Price (₹)</label><input v-model.number="form.cost_price" type="number" step="0.01" class="pulsara-input font-mono" /></div>
                    <div><label class="pulsara-label">Selling Price (₹)</label><input v-model.number="form.selling_price" type="number" step="0.01" class="pulsara-input font-mono" /></div>
                </div>
                <div><label class="pulsara-label">Min Stock Level</label><input v-model.number="form.min_stock_level" type="number" class="pulsara-input w-32" /></div>
