...
<TenantLayout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
        <div class="card"><div class="heyd2c-label">Vendor</div><div class="text-white text-[14px]">{{ po.vendor?.name }}</div><div v-if="po.vendor?.gstin" class="text-[11px] text-ink-3 font-mono mt-1">GSTIN: {{ po.vendor.gstin }}</div></div>
        <div class="card"><div class="heyd2c-label">Expected</div><div class="text-white text-[14px]">{{ dateFmt(po.expected_date) }}</div></div>
        <div class="card"><div class="heyd2c-label">Total Amount</div><div class="text-emerald text-[18px] font-mono font-bold">{{ fmt(po.total_amount) }}</div></div>
    </div>
    ...
    <div v-if="po.notes" class="card mt-5"><div class="heyd2c-label">Notes</div><p class="text-[13px] text-ink-2 mt-1">{{ po.notes }}</p></div>
</TenantLayout>
</template>
