    ...
    <form @submit.prevent="submit" class="space-y-5">
        ...
        <div class="card">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="heyd2c-label">PO Number</label>
                    <input v-model="form.po_number" class="heyd2c-input font-mono" />
                    <div v-if="form.errors.po_number" class="mt-1 text-[11px] text-rose">{{ form.errors.po_number }}</div>
                </div>
                <div>
                    <label class="heyd2c-label">Vendor</label>
                    <select v-model="form.vendor_id" class="heyd2c-input">
                ...
                <div>
                    ...
                    <input v-model="form.order_date" type="date" class="heyd2c-input" />
                </div>
                <div>
                    <label class="heyd2c-label">Expected Date</label>
            ...
            <table class="w-full text-[12px]">
                ...
                <tbody class="divide-y divide-frost-1">
                    <tr v-for="(item, i) in form.items" :key="i" class="hover:bg-brand-600/5">
                        <td class="px-4 py-2"><input v-model="item.product_name" class="heyd2c-input text-[12px]" placeholder="Product name" /></td>
                        <td class="px-4 py-2"><input v-model="item.sku" class="heyd2c-input text-[12px] font-mono" placeholder="SKU" /></td>
                        <td class="px-4 py-2"><input v-model.number="item.quantity" type="number" min="1" class="heyd2c-input text-[12px] text-center" /></td>
                        <td class="px-4 py-2"><input v-model.number="item.unit_price" type="number" step="0.01" class="heyd2c-input text-[12px] text-right font-mono" placeholder="0.00" /></td>
                        <td class="px-4 py-2"><input v-model.number="item.tax_rate" type="number" step="0.5" class="heyd2c-input text-[12px] text-center font-mono" /></td>
        ...
        <div class="card">
            ...
            <textarea v-model="form.notes" class="heyd2c-input" rows="2" placeholder="Optional notes…"></textarea>
        </div>
