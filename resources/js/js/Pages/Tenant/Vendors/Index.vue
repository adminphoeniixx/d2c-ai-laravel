    ...
    <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="showForm = false">
        <div class="card w-full max-w-lg mx-4 max-h-[85vh] overflow-y-auto">
            ...
            <form @submit.prevent="submit" class="space-y-3">
                <div><label class="heyd2c-label">Company Name *</label><input v-model="form.name" class="heyd2c-input" /><div v-if="form.errors.name" class="mt-1 text-[11px] text-rose">{{ form.errors.name }}</div></div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="heyd2c-label">GSTIN</label><input v-model="form.gstin" class="heyd2c-input font-mono" placeholder="22AAAAA0000A1Z5" /></div>
                    <div><label class="heyd2c-label">Contact Person</label><input v-model="form.contact_person" class="heyd2c-input" /></div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <div><label class="heyd2c-label">Pincode</label><input v-model="form.pincode" class="heyd2c-input font-mono" /></div>
                </div>
