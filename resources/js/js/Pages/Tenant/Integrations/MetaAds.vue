    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div v-if="account && account.status === 'connected'" class="card space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 text-[13px]">
                <div><span class="heyd2c-label">Ad Account</span><div class="text-ink font-mono">{{ account.ad_account_name || account.ad_account_id }}</div></div>
        ...
        <div v-else class="space-y-4">
            ...
            <div v-if="showManual" class="card">
                ...
                <form @submit.prevent="manualSubmit" class="space-y-3">
                    <div>
                        <label class="heyd2c-label">Long-Lived Access Token</label>
                        <input v-model="manualForm.access_token" class="heyd2c-input font-mono text-[11px]" placeholder="EAA..." />
                        <div v-if="manualForm.errors.access_token" class="mt-1 text-[11px] text-rose">{{ manualForm.errors.access_token }}</div>
                    </div>
                    <div>
                        <label class="heyd2c-label">Ad Account ID (without act_ prefix)</label>
                        <input v-model="manualForm.ad_account_id" class="heyd2c-input font-mono" placeholder="123456789" />
