    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div v-if="account && account.status === 'connected'" class="card space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 text-[13px]">
                <div><span class="heyd2c-label">Customer ID</span><div class="text-ink font-mono">{{ account.customer_id }}</div></div>
                <div><span class="heyd2c-label">Connected At</span><div class="text-ink">{{ dateFmt(account.connected_at) }}</div></div>
                <div><span class="heyd2c-label">Last Synced</span><div class="text-ink">{{ dateFmt(account.last_synced_at) }}</div></div>
            </div>
        ...
        <div v-else class="space-y-4">
            ...
            <div v-if="showManual" class="card">
                ...
                <form @submit.prevent="manualSubmit" class="space-y-3">
                    <div>
                        <label class="heyd2c-label">Access Token</label>
                        <input v-model="manualForm.access_token" class="heyd2c-input font-mono text-[11px]" placeholder="ya29...." />
                    </div>
                    <div>
                        <label class="heyd2c-label">Refresh Token</label>
                        <input v-model="manualForm.refresh_token" class="heyd2c-input font-mono text-[11px]" placeholder="1//0..." />
                    </div>
                    <div>
                        <label class="heyd2c-label">Customer ID (no dashes)</label>
                        <input v-model="manualForm.customer_id" class="heyd2c-input font-mono" placeholder="1234567890" />
                    </div>
