        ...
        <div class="max-w-3xl">
            ...
            <div v-if="account && account.status === 'connected'" class="card mb-4 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <Link :href="route('tenant.integrations.woo.sync', { tenant: slug })" method="post" as="button" class="btn btn-primary btn-sm">Sync Orders</Link>
            ...
            <div v-else class="card mb-4">
                ...
                <form v-if="!showManual" class="space-y-3" @submit.prevent="oauthForm.post(route('tenant.integrations.woo.connect', { tenant: slug }))">
                    <label class="heyd2c-label">Store URL</label>
                    <input v-model="oauthForm.shop_url" placeholder="https://yourstore.com" class="heyd2c-input" />
                ...
                <form v-else class="space-y-3" @submit.prevent="manualForm.post(route('tenant.integrations.woo.manual', { tenant: slug }))">
                    ...
                    <div>
                        <label class="heyd2c-label">Store URL</label>
                        <input v-model="manualForm.shop_url" placeholder="https://yourstore.com" class="heyd2c-input" />
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="heyd2c-label">Consumer key</label>
                            <input v-model="manualForm.consumer_key" placeholder="ck_…" class="heyd2c-input font-mono" />
                        </div>
                        <div>
                            <label class="heyd2c-label">Consumer secret</label>
                            <input v-model="manualForm.consumer_secret" placeholder="cs_…" class="heyd2c-input font-mono" />
                        </div>
                    </div>
